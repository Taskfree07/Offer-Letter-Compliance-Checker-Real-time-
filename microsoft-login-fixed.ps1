# ========================================
# Microsoft OAuth Login - Fixed Version
# Works without Azure App Registration changes
# ========================================
...
$redirectUri = "http://localhost:8400"  # Using localhost redirect
...
try {
    ...
    # Try with localhost redirect URI first
    try {
        ...
        Write-Host "Localhost redirect failed, trying device code flow..." -ForegroundColor Yellow
        
        # Fallback to device code flow (works without redirect URI)
        $msalToken = Get-MsalToken `
            -ClientId $clientId `
            -TenantId $tenantId `
            -Scopes @("User.Read") `
...
"@ | Out-File -FilePath $fileName -Encoding UTF8
    ...
    Write-Host "3. If redirect URI error, ask IT to add 'http://localhost:8400' to Azure App Registration" -ForegroundColor White
    Write-Host "4. Try the device code flow alternative (see docs)" -ForegroundColor White
    Write-Host "5. Contact IT support if issue persists`n" -ForegroundColor White
