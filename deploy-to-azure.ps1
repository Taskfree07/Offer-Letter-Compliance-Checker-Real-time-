...
Write-Host "✅ Storage configured`n" -ForegroundColor Green
...
Write-Host "✅ ACR created and logged in`n" -ForegroundColor Green

# Step 5: Build and Push Docker Images
Write-Host "🏗️ Step 5: Building and pushing Docker images..." -ForegroundColor Green

# ONLYOFFICE (pull and push official image first - needed for backend config)
...
Write-Host "  ✅ ONLYOFFICE image pushed" -ForegroundColor Green

# Backend (build without frontend URL - will set via env vars)
...
# Frontend (build placeholder - will rebuild after getting backend URL)
...
Write-Host "  ✅ Frontend temporary image built`n" -ForegroundColor Green
...
Write-Host "✅ ONLYOFFICE deployed at: https://$ONLYOFFICE_URL`n" -ForegroundColor Green
...
az containerapp create `
  ...
  --env-vars `
    ...
    BACKEND_BASE_URL=https://backend.{{ BACKEND_FQDN }}
...
Write-Host "✅ Backend deployed at: https://$BACKEND_URL`n" -ForegroundColor Green

# Step 8: Deploy Frontend (React)
Write-Host "⚛️ Step 8: Deploying React Frontend..." -ForegroundColor Green
...
Write-Host "✅ Frontend deployed at: https://$FRONTEND_URL`n" -ForegroundColor Green
